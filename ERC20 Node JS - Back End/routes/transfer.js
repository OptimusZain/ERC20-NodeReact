...
const router = express.Router();
  ...
  router.post('/', async (req,res) => {
    ...
    // console.log(acc);
    res.send(await Contract.methods.transfer(req.body.recipient, req.body.amount).send({from : acc[0]}));
    });
  
    console.log("Contract Address:" , MyContract.networks[networkId].address);
  }
